#!/usr/bin/env python3
"""
CSV Historical Data Collection Script

This script collects historical NBA data using the Tank01 API and saves it to CSV files.

Usage:
    python collect_csv_data.py --start-date 20240101 --end-date 20240131
    python collect_csv_data.py --season 2024  # Collect full season
    python collect_csv_data.py --recent-days 30  # Last 30 days
"""
...
from csv_collector import CSVDataCollector, CollectionConfig
...
def setup_logging(verbose: bool = False):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('csv_collection.log')
        ]
    )
...
def main():
    parser = argparse.ArgumentParser(description="Collect NBA data to CSV files")
    ...
    parser.add_argument("--data-dir", default="data", help="Directory to store CSV files (default: data)")
    ...
    parser.add_argument("--odds-only", action="store_true", help="Only collect betting odds")
    ...
    logger.info(f"Data will be stored in: {args.data_dir}")
    ...
    try:
        ...
        # Create collector
        collector = CSVDataCollector(api, config)

        logger.info("Starting CSV data collection...")
        ...
        if args.games_only:
            ...
            logger.info("Collecting betting odds only...")
            collector.collect_betting_odds_for_date_range()
        else:
            # Run full collection
        ...
        # Print summary
        logger.info("Getting collection summary...")
        summary = collector.get_collection_summary()

        logger.info("Collection Summary:")
        for data_type, stats in summary.items():
            if 'total_records' in stats:
                logger.info(f"  {data_type}: {stats['total_records']} records, {stats['file_size_mb']} MB")
            else:
                logger.info(f"  {data_type}: {stats}")

        logger.info("CSV data collection completed successfully!")
